...
$(document).ready(function(){
    ...
    $("#sortable1, #sortable4, #sortable5, #sortable3 ").sortable({
	...
	$('form#createLineBasicChart').submit(function(e){
		...
		if($('ul#sortable4').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the X Value',
				type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable5').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the Y Value',
				type: 'error'
			});
			return false;
		}
		...
		$('ul#sortable4').find('li').each(function(){
			xValueColumn[$(this).find('span.columnText').text()] = $(this).data('id');
		});

		var yValueColumn = {};

		$('ul#sortable5').find('li').each(function(){
			yValueColumn[$(this).find('span.columnText').text()] = $(this).data('id');
		});
		...
		params.push({name: 'xValueColumn', value: JSON.stringify(xValueColumn) });
		params.push({name: 'yValueColumn', value: JSON.stringify(yValueColumn) });
