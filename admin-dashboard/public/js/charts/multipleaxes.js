...
$(document).ready(function(){
    ...
    $("#sortable1, #sortable4, #sortable3, #sortable5, #sortable6").sortable({
	...
	$('form#createChart').submit(function(e){
		...
		if($('ul#sortable4').find('li').length == 0){
        	new PNotify({
			    title: 'Error!',
			    text: 'You have to select atleast one column for the column section',
			    type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable5').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the line 1 section',
				type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable6').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the line 2 section',
				type: 'error'
			});
			return false;
		}
		...
		$('ul#sortable6').find('li').each(function(){
			lineColumn2[$(this).find('span.columnText').text()] = $(this).data('id');
		});

		var columns = {};
		$('ul#sortable4').find('li').each(function(){
			columns[$(this).find('span.columnText').text()] = $(this).data('id');
		});
		...
		params.push({name: 'lineColumn1', value: JSON.stringify(lineColumns) });
		params.push({name: 'lineColumn2', value: JSON.stringify(lineColumn2) });
		params.push({name: 'columnColumn', value: JSON.stringify(columns) });
