...
$(document).ready(function(){
	...
	$('form#createChart').submit(function(e){
		...
		if($('ul#sortable3').find('li').length == 0){
        	new PNotify({
			    title: 'Error!',
			    text: 'You have to select a column for the X Axis',
			    type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable4').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select a column for the Y Axis',
				type: 'error'
			});
			return false;
		}
		...
		var yAxisCategories = $(this).find('ul#sortable4').find('li').data('id');
		...
		params.push({name: 'yAxisCategories', value: yAxisCategories });
