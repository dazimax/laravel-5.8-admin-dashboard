...
$(document).ready(function(){
    ...
    $("#sortable1, #sortable4, #sortable3, #sortable5, #sortable6, #sortable7, #sortable8").sortable({
      ...
      receive: function( event, ui ) {
        if ($('#sortable3').children().length > 1) {
		  ...
		  if ($('#sortable6').children().length > 1) {
			  $(ui.sender).sortable('cancel');
		  }
		  if ($('#sortable7').children().length > 1) {
			  $(ui.sender).sortable('cancel');
		  }
		  if ($('#sortable8').children().length > 1) {
			  $(ui.sender).sortable('cancel');
		  }
	...
	$('form#createChart').submit(function(e){
		...
		if($('ul#sortable4').find('li').length == 0){
        	new PNotify({
			    title: 'Error!',
			    text: 'You have to select atleast one column for the Minimum Value section',
			    type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable5').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the Maximum Value section',
				type: 'error'
			});
			return false;
		}

		if($('ul#sortable6').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the Upper Quartile section',
				type: 'error'
			});
			return false;
		}
		if($('ul#sortable7').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the Median section',
				type: 'error'
			});
			return false;
		}
		if($('ul#sortable7').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select atleast one column for the Lower Quartile section',
				type: 'error'
			});
			return false;
		}
		...
		var column3 = {};
		$('ul#sortable6').find('li').each(function(){
			column3[$(this).find('span.columnText').text()] = $(this).data('id');
		});

		var column4 = {};
		$('ul#sortable7').find('li').each(function(){
			column4[$(this).find('span.columnText').text()] = $(this).data('id');
		});

		var column5 = {};
		$('ul#sortable8').find('li').each(function(){
			column5[$(this).find('span.columnText').text()] = $(this).data('id');
		});

		var theoMeanValue = $('input[name="theomean"]').val();
		...
		params.push({name: 'column3', value: JSON.stringify(column3) });
		params.push({name: 'column4', value: JSON.stringify(column4) });
		params.push({name: 'column5', value: JSON.stringify(column5) });
		params.push({name: 'theoMean', value: theoMeanValue });
