...
$(document).ready(function(){
    ...
    $("#sortable1, #sortable2, #sortable3,#sortable4").sortable({
      ...
      receive: function( event, ui ) {
		  ...
		  if ($('#sortable2').children().length > 2) {
			  $(ui.sender).sortable('cancel');
		  }
      }
    }).disableSelection();
	...
	$('form#createChart').submit(function(e){
		...
		if($('ul#sortable3').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select a column for x categories',
				type: 'error'
			});
			return false;
		}
		...
		if($('ul#sortable4').find('li').length == 0){
			new PNotify({
				title: 'Error!',
				text: 'You have to select a column for Line',
				type: 'error'
			});
			return false;
		}
		...
		var arealine = $(this).find('ul#sortable4').find('li').data('id');
		...
		params.push({name: 'arealine', value: arealine });
